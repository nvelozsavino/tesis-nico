...
void Spectrum::initSpectrum(float lamdaStart, float lamdaEnd,unsigned int arraySize){
	...
	endLamda=lamdaEnd;
	valores.create(1,size,CV_32FC1);
...
void Spectrum::setGausian(float amplitud, float mean, float sigma, float add){
	...
	for (unsigned int i=0;i<size;i++){
		l=i2lamda(i);
		sum=add*get2D32F(valores,i,0,0);
		set2D32F(valores,i,0,0,sum+amp*amplitud*exp(-(l-mean)*(l-mean)/(2*sigma*sigma)));
...
void Spectrum::setPlain(float lamdastart, float lamdaend,float amplitud=1){
	...
	for (i=is;i<ie+1;i++){
		set2D32F(valores,i,0,0,abs(amplitud));
	}
	lamda2freq();
}
...
float Spectrum::integrarEspectro(void){
	...
	result=Io[0]-(get2D32F(valoresFreq,0,0,0) + get2D32F(valoresFreq,valoresFreq.cols-1,0,0))/2;
   ...
   // printf("size_temp %d\n",size_temp);
	...
	for (i=0;i<size_temp;i++){
		...
		if (isf<0 || ipf<0 || ipf>src2.size-1 || isf>src1.size-1){
            ...
            if (isu!=isd){
                m=(get2D32F(valores,isu,0,0)-get2D32F(valores,isd,0,0))/(isu-isd);
                b=get2D32F(valores,isd,0,0);
                mults=m*(isf-isd)+b;
            } else {
                mults=get2D32F(valores,isu,0,0);
            }
            ...
            if (isu!=isd){
                ...
                m=(get2D32F(src2.valores,isu,0,0)-get2D32F(src2.valores,isd,0,0))/(isu-isd);
                b=get2D32F(src2.valores,isd,0,0);
                multp=m*(ipf-isd)+b;
            } else {
                multp=get2D32F(src2.valores,isu,0,0);
            }

            set2D32F(dst.valores,i,0,0,mults*multp);
        }
	}
	dst.lamda2freq();
...
//	float resTemp;
    ...
    res1=(src1.cols)/endFreq1;
    res2=(src2.cols)/endFreq2;
    ...
    dst->create(1,size_temp,CV_32FC1);


	for (unsigned int i=0;i<size_temp;i++){

		f=i*(*endFreqDst)/(size_temp-1);
		is1=f*(src1.cols-1)/endFreq1;
        is2=f*(src2.cols-1)/endFreq2;
        ...
        if (is1>(src1.cols-1)){
            ...
            i1=(int)is1;
            if (i1!=src1.cols-1){
                yd1=get2D32F(src1,i1,0,0);
                yu1=get2D32F(src1,i1+1,0,0);
                mult1=(yu1-yd1)*(is1-i1)+yd1;
            } else {
                mult1=get2D32F(src1,i1,0,0);
            }
        }
        ...
        if (is2>(src2.cols-1)){
            ...
            i2=(int)is2;
            if (i2!=src2.cols-1){
                yd2=get2D32F(src2,i2,0,0);
                yu2=get2D32F(src2,i2+1,0,0);
                mult2=(yu2-yd2)*(is2-i2)+yd2;
            } else {
                mult2=get2D32F(src2,i2,0,0);
            }
        }
        set2D32F((*dst),i,0,0,mult1*mult2);
	}
}
...
void Spectrum::lamda2freq(void){
    ...
    float ff=LIGHT_SPEED/startLamda;
    valoresFreq.create(1,N,CV_32FC1);
    valoresFreq.setTo(Scalar::all(0));
    for (int x=1;x<valoresFreq.cols;x++){
        float f=x*(ff/(valoresFreq.cols-1));
        set2D32F(valoresFreq,x,0,0,f2val(f));
    }
...
float Spectrum::f2val(float freq){
    ...
    float i=(lamda-startLamda)*(valores.cols-1)/(endLamda-startLamda);
    int id=(int)i;
    if (id>=valores.cols){
        return 0;
    }
    if (id!=valores.cols-1){
        float yid=get2D32F(valores,id,0,0);
        float yid1=get2D32F(valores,id+1,0,0);
        return ((yid1-yid)*(i-id)+yid);
    }else {
        return get2D32F(valores,id,0,0);
    }
}
